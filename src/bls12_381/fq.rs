...
const MULTIPLICATIVE_GENERATOR: Fq = Fq::from_raw_unchecked([0x02, 0x00, 0x00, 0x00, 0x00, 0x00]);
...
// L=(k^3-1).roots();L
const ZETA: Fq = Fq::from_raw_unchecked([
    0x30f1361b798a64e8,
    0xf3b8ddab7ece5a2a,
    0x16a8ca3ac61577f7,
    0xc26a2ff874fd029b,
...
impl Fq {
    ...
    pub fn from_bytes(bytes: &[u8; 48]) -> CtOption<Fq> {
    ...
    pub(crate) fn random(mut rng: impl RngCore) -> Fq {
    ...
    fn from_u768(limbs: [u64; 12]) -> Fq {
        ...
        // constant `R2` or `R3`.
        let d1 = Fq([limbs[11], limbs[10], limbs[9], limbs[8], limbs[7], limbs[6]]);
        let d0 = Fq([limbs[5], limbs[4], limbs[3], limbs[2], limbs[1], limbs[0]]);
    ...
    /// canonical.
    pub const fn from_raw_unchecked(v: [u64; 6]) -> Fq {
        Fq(v)
    }
    ...
    const fn subtract_p(&self) -> Fq {
        ...
        Fq([r0, r1, r2, r3, r4, r5])
    }

    #[inline]
    pub const fn add(&self, rhs: &Fq) -> Fq {
        ...
        (Fq([d0, d1, d2, d3, d4, d5])).subtract_p()
    }

    #[inline]
    pub const fn neg(&self) -> Fq {
    ...
    #[inline]
    pub const fn sub(&self, rhs: &Fq) -> Fq {
        (&rhs.neg()).add(self)
    }
    ...
    pub(crate) fn sum_of_products<const T: usize>(a: [Fq; T], b: [Fq; T]) -> Fq {
        ...
        (Fq([u0, u1, u2, u3, u4, u5])).subtract_p()
    }
    ...
    pub(crate) const fn montgomery_reduce(
        ...
        // Attempt to subtract the modulus, to ensure the value
        // is smaller than the modulus.
        (Fq([r6, r7, r8, r9, r10, r11])).subtract_p()
    }

    #[inline]
    pub const fn mul(&self, rhs: &Fq) -> Fq {
...
fn test_conditional_selection() {
    let a = Fq([1, 2, 3, 4, 5, 6]);
    let b = Fq([7, 8, 9, 10, 11, 12]);
...
fn test_equality() {
    ...
    assert!(is_equal(&Fq([1, 2, 3, 4, 5, 6]), &Fq([1, 2, 3, 4, 5, 6])));

    assert!(!is_equal(&Fq([7, 2, 3, 4, 5, 6]), &Fq([1, 2, 3, 4, 5, 6])));
    assert!(!is_equal(&Fq([1, 7, 3, 4, 5, 6]), &Fq([1, 2, 3, 4, 5, 6])));
    assert!(!is_equal(&Fq([1, 2, 7, 4, 5, 6]), &Fq([1, 2, 3, 4, 5, 6])));
    assert!(!is_equal(&Fq([1, 2, 3, 7, 5, 6]), &Fq([1, 2, 3, 4, 5, 6])));
    assert!(!is_equal(&Fq([1, 2, 3, 4, 7, 6]), &Fq([1, 2, 3, 4, 5, 6])));
    assert!(!is_equal(&Fq([1, 2, 3, 4, 5, 7]), &Fq([1, 2, 3, 4, 5, 6])));
}
...
fn test_from_bytes() {
    ...
    for _ in 0..100 {
        a = a.square();
        let tmp = a.to_bytes();
        let b = Fq::from_bytes(&tmp).unwrap();
    ...
    assert!(bool::from(Fq::from_bytes(&[0xff; 48]).is_none()));
}
